...
import { htmlBlocoPagamentoEncomenda } from './modal-finalizar.js';
...
export function htmlFormularioEncomenda({
  ...
  const valorSinal = Number(formulario.sinal) || 0;
  const blocoSinal = !edicao && valorSinal > 0
    ? `${htmlBlocoPagamentoEncomenda({
        valorACobrar: valorSinal,
        forma: formulario.forma,
        recebido: formulario.recebido,
        inputId: 'encomenda-sinal-recebido',
      })}
      <p class="campo-erro">${escapar(errosCampos.forma || '')}</p>`
    : '';
      ...
      <div class="form-modal-corpo">
        ...
        <label class="produtos-toggle">
          <span class="produtos-toggle-texto">
            <span class="produtos-toggle-titulo">Paga depois / sem sinal</span>
            <span class="produtos-toggle-ajuda">Cliente acerta na entrega (ex.: fim de semana)</span>
          </span>
          <input type="checkbox" id="encomenda-paga-depois" class="produtos-toggle-input"${formulario.pagaDepois ? ' checked' : ''}>
          <span class="produtos-toggle-pista" aria-hidden="true"></span>
        </label>

        <label>Sinal
          <input type="number" id="encomenda-sinal" name="sinal" min="0" step="0.01" value="${escapar(formulario.sinal)}"${formulario.pagaDepois ? ' disabled' : ''}>
        </label>
